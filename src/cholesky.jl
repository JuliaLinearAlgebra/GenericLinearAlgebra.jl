module CholeskyModule
	...
	function cholUnblocked!{T<:Number}(A::AbstractMatrix{T}, ::Type{Sym{:L}})
		n = chksquare(A)
		A[1,1] = sqrt(A[1,1])
		if n > 1
			a21 = view(A, 2:n, 1)
			...
			cholUnblocked!(A22, Sym{:L})
		end
		A
	end

	function cholBlocked!{T<:Number}(A::AbstractMatrix{T}, ::Type{Sym{:L}}, blocksize::Integer)
		n = chksquare(A)
		mnb = min(n, blocksize)
		A11 = view(A, 1:mnb, 1:mnb)
		cholUnblocked!(A11, Sym{:L})
		if n > blocksize
			...
			rankUpdate!(-one(T), A21, Symmetric(A22, :L))
			cholBlocked!(A22, Sym{:L}, blocksize)
		end
		A
	end
end